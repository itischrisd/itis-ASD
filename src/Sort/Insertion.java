...
/*
    ...
    większą lub równą zero. Stąd wynika, że i stanie się większe lub równe len w skończonej liczbie iteracji. Jeśli
    sterowanie przepływu programu wejdzie do wewnętrznej pętli, to zakończy się ona, gdy co najmniej zmienna j osiągnie
    wartość mniejszą lub równą zero. j w każdej iteracji zewnętrznej pętli inicjowane jest wartością i (skończoną liczbą
    naturalną) i zmniejszane o 1 w każdej iteracji pętli wewnętrznej. Stąd wynika, że wewnętrzna pętla również zatrzyma
    się w skończonej liczbie iteracji.
    ...
    3. Krok indukcyjny.
    Teza indukcyjna:
    Dla (k+1)-tej iteracji pętli (dla pewnego k > 1) niezmiennik pętli jest spełniony, czyli każdy element
    arr[j] dla j z zakresu [0, k+1) jest na właściwym miejscu w posortowanej części arr[0:k+1).
    Dowód indukcyjny:
